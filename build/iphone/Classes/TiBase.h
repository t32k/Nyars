...
// in simulator we redefine to format for Nyars Developer console
...
{\
	NSString * exceptionName = [@"org.nyars." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.nyars";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UINyarsNativeItemNone = -1, 
	UINyarsNativeItemSpinner = -2,
	UINyarsNativeItemProgressBar = -3,
	
	UINyarsNativeItemSlider = -4,
	UINyarsNativeItemSwitch = -5,
	UINyarsNativeItemMultiButton = -6,
	UINyarsNativeItemSegmented = -7,
	
	UINyarsNativeItemTextView = -8,
	UINyarsNativeItemTextField = -9,
	UINyarsNativeItemSearchBar = -10,
	
	UINyarsNativeItemPicker = -11,
	UINyarsNativeItemDatePicker = -12,
	
	UINyarsNativeItemInfoLight = -13,
	UINyarsNativeItemInfoDark = -14,
	
	UINyarsNativeItemDisclosure = -15,
	
	UINyarsNativeItemContactAdd = -16
};
